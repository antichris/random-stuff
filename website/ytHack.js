function ytHack() {
try {
	this.div = { id : 'YThackDiv' };
	this.fmtTitles = {0:'360p',512000:'AVC1',640000:'480p',2000000:'720p(HD)',4000000:'1080p(HD)'};

} catch (err) { block = 'videoTitle'; alert(block+'\n'+err) } try {

	this.getVideoTitle = function () {
		return document.getElementById('watch-headline-title').getElementsByTagName('span')[0].title;
		};
	this.yt = { videoTitle : this.getVideoTitle() };

} catch (err) { block = 'flashVars'; alert(block+'\n'+err) } try {

	this.getFlashVars = function (moviePlayer) {
		if (typeof(moviePlayer)!='object')
			moviePlayer = document.getElementById('movie_player');
		var flashVars = {}, flashVar = [];
...
} catch (err) { block = 'urlmap'; alert(block+'\n'+err) } try {

	this.prepFmtMap = function () {
		var fmtMap = decodeURIComponent(this.yt.fmtMap), re=/\D+/;
		fmtMap = fmtMap.split(',');
		for (key in fmtMap) {
...
} catch (err) { block = 'fmtmap'; alert(block+'\n'+err) } try {
...
} catch (err) { block = 'titlediv'; alert(block+'\n'+err) } try {
...
} catch (err) { block = 'linksdiv'; alert(block+'\n'+err) } try {
...
} catch (err) { block = 'add'; alert(block+'\n'+err) } try {
...
} catch (err) { block = 'tiny fns'; alert(block+'\n'+err) } try {

	this.add();
	this.show();
} catch (err) { block = 'init'; alert(block+'\n'+err) }
	};
